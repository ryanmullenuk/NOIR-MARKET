...
function sound(kind){ if(kind==='positive'){tone(660,.07,'sine',.035);tone(990,.09,'sine',.03,.07)} else if(kind==='negative'){tone(170,.11,'sawtooth',.03);tone(105,.13,'sawtooth',.026,.09)} else if(kind==='travel'){tone(740,.12,'sine',.035);tone(554,.16,'sine',.033,.15)} }
...
function buyModal(){modal('Buy',`<div class="modal-money"><span>Current funds</span><strong>${money(s.cash)}</strong><em>Storage ${used()}/${totalSpace()}</em></div><div class="trade-grid">${drugs.map(([name,icon])=>`<div class="trade-tile"><div class="trade-title"><b>${icon}</b><strong>${name}</strong></div><p>${money(s.prices[name])} each</p><p>Available: ${s.supply[name]}</p><p>Owned: ${s.inv[name]}</p>${qtyControl(name,'buy',s.supply[name])}<button type="button" class="buy" data-buydrug="${name}">BUY</button></div>`).join('')}</div><div class="trade-footer"><span>Cash ${money(s.cash)}</span><span>Storage ${used()}/${totalSpace()}</span></div>`); setTimeout(()=>{bindQtyButtons('buy');document.querySelectorAll('[data-buydrug]').forEach(b=>b.onclick=()=>{let name=b.dataset.buydrug,q=+qtyInput('buy',name).value||0,cost=q*s.prices[name]; if(q<1){errorMsg('ENTER QUANTITY');return;} if(q>s.supply[name]){errorMsg('NOT ENOUGH STOCK');return;} if(cost>s.cash){errorMsg('INSUFFICIENT FUNDS');return;} if(q>totalSpace()-used()){errorMsg('INSUFFICIENT STORAGE');return;} s.cash-=cost; s.inv[name]+=q; s.supply[name]-=q; s.notice=`Bought ${q} units of ${name}.`; save(); draw(); success(`Bought ${q} ${name}`); buyModal();});},0)}
function sellModal(){let owned=drugs.filter(([name])=>s.inv[name]>0); modal('Sell',`<div class="modal-money"><span>Current funds</span><strong>${money(s.cash)}</strong><em>Storage ${used()}/${totalSpace()}</em></div>${owned.length?`<div class="trade-grid">${owned.map(([name,icon])=>`<div class="trade-tile"><div class="trade-title"><b>${icon}</b><strong>${name}</strong></div><p>Owned: ${s.inv[name]}</p><p>Price: ${money(s.prices[name])}</p><p>Sell all: <strong>${money(s.inv[name]*s.prices[name])}</strong></p>${qtyControl(name,'sell',s.inv[name])}<p class="tile-value">Selected: <b data-sellvalue="${name}">£0</b></p><button type="button" class="sell" data-selldrug="${name}">SELL</button></div>`).join('')}</div>`:'<p class="subtle">You have no stock to sell.</p>'}<div class="trade-footer"><span>Cash ${money(s.cash)}</span><span>Owned lines ${owned.length}</span></div>`); setTimeout(()=>{bindQtyButtons('sell');document.querySelectorAll('[id^="qty-sell-"]').forEach(i=>i.oninput=updateSellValues);document.querySelectorAll('[data-selldrug]').forEach(b=>b.onclick=()=>{let name=b.dataset.selldrug,q=+qtyInput('sell',name).value||0; if(q<1){errorMsg('ENTER QUANTITY');return;} if(q>s.inv[name]){errorMsg('NOT ENOUGH STOCK');return;} s.cash+=q*s.prices[name]; s.inv[name]-=q; s.notice=`Sold ${q} units of ${name}.`; save(); draw(); success(`Sold ${q} ${name}`); sellModal();});updateSellValues();},0)}
...
function stay(){sound('positive');haptic();nextDay(`You stay in ${places[s.city][0]}.`,true)}
...
function save(){localStorage.setItem('noir_market_v12',JSON.stringify(s))} function load(){let x=localStorage.getItem('noir_market_v12')||localStorage.getItem('noir_market_v9')||localStorage.getItem('noir_market_v6')||localStorage.getItem('noir_market_v5')||localStorage.getItem('noir_market_v4'); if(x){s=JSON.parse(x);draw();return false;} newGame(false);return true;}
function particles(){
